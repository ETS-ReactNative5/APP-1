...
(async () => {
  ...
  if (emulators.length > 0) {
    const answers = await (0, _inquirer.prompt)(questions);
...
function startEmulator(answers) {
  try {
    ...
    logMessage(MESSAGE_TYPES.NOTIFY, `${answers.emulator} is being started. You can close the device with 'X'-button on the toolbar of the device.`);
    const options = {};
    ...
    const runOnEmulator = (0, _child_process.spawnSync)((0, _path.normalize)((0, _path.join)(process.env.ANDROID_HOME, '/emulator/emulator')), ['-avd', answers.emulator, answers.wipeData === 'Yes' ? '-wipe-data' : '-no-snapshot'], options);
    console.log((0, _chalk.yellow)(runOnEmulator.stdout.toString())); // Check if there is an error and throw it
    ...
    if (e.message.includes('Cannot read property \'toString\' of null')) {
      errorMessage = 'It looks like the `emulator` has not been set correct in your environment variables. ' + 'Please check this article on how to fix it.' + '\n\n  https://bit.ly/2XD94gV';
    } else if (!(0, _fs.existsSync)(process.env.ANDROID_HOME)) {
...
function getEmulators() {
  const emulators = (0, _child_process.execFileSync)((0, _path.normalize)((0, _path.join)(process.env.ANDROID_HOME, '/emulator/emulator')), ['-list-avds'], {
    encoding: 'utf8'
  }).replace(/\n$/, '').split('\n');
...
function logMessage(type, message) {
  const messageType = type === MESSAGE_TYPES.NOTIFY ? _chalk.cyan : _chalk.red;
